...
class Handler:
    ...
    def handle_message(self, mess):
        ...
        if mess['body'] == "!help":
            ...
            self.bot.send_message(ID, "Список команд, разрешенных в !v:\n" + str(self.safe_dict.keys()))
        elif mess['uid'] == 136776175 and mess['body'] == "!stop":
            self.bot.send_message(ID, "--ок, ухажу--")
            os.kill(os.getpid(), signal.XXXX)
        elif "[id" + str(self.me) + "|" in mess['body']:
            user = self.bot.get_user(mess['uid'])
            self.bot.send_message(ID, "[id" + str(mess['uid']) + "|" + user['first_name'] + " " + user[
                'last_name'] + "], отъебись блять")
        elif mess['body'] == "!ping":
            ...
            try:
                ...
                if len(o.split()) == 0:
                    self.bot.send_message(ID, "[id" + mess['uid'] + "|ПИДОР]!")
                else:
        ...
        if mess['uid'] != self.me:
            ...
            if mess['uid'] == 445077792:
                self.nahui.append([self.bot.send_message(ID, "Юра, иди нахуй"), time.time() + 20])
